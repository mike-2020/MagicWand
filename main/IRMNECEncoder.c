...
static size_t rmt_encode_ir_nec(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    case 0: // send leading code
        ...
        //ESP_LOGW(TAG, "encoded_symbols 1=%d", encoded_symbols);
    ...
    case 1: // send address
        ...
        //ESP_LOGW(TAG, "encoded_symbols 2=%d, scan_code->address=0x%X, session_state=0x%X", encoded_symbols, scan_code->address, session_state);
        if (session_state & RMT_ENCODING_MEM_FULL) {
            state |= RMT_ENCODING_MEM_FULL;
            goto out; // yield if there's no free space to put other encoding artifacts
        }
    ...
    case 2: // send command
        ...
        //ESP_LOGW(TAG, "encoded_symbols 3=%d, scan_code->command=0x%X, session_state=0x%X", encoded_symbols, scan_code->command, session_state);
        if (session_state & RMT_ENCODING_MEM_FULL) {
            state |= RMT_ENCODING_MEM_FULL;
            goto out; // yield if there's no free space to put other encoding artifacts
        }
    ...
    case 3: // send ending code
        ...
        //ESP_LOGW(TAG, "encoded_symbols 4=%d", encoded_symbols);
        if (session_state & RMT_ENCODING_MEM_FULL) {
            state |= RMT_ENCODING_MEM_FULL;
            goto out; // yield if there's no free space to put other encoding artifacts
        }
...
out:
    *ret_state = state;
    //ESP_LOGW(TAG, "encoded_symbols 5=%d", encoded_symbols);
