...
{
    esp_hidd_dev_battery_set(s_ble_hid_param.hid_dev, level);
}
...
{
    RemoteController::getInstance()->onHostConnected();
...
{
    RemoteController::getInstance()->onHostDisconnected();
    //if (s_ble_hid_param.task_hdl) {
...
#if CONFIG_BT_NIMBLE_ENABLED
extern "C" void ble_hid_device_host_task(void *param)
{
...
int ble_main(void)
{
    ...
    ret = esp_nimble_enable((void*)ble_hid_device_host_task);
    if (ret) {
